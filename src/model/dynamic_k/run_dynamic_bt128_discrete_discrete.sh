...
# 创建日志目录
mkdir -p dynamic_logs_H

# ===== 设置实验标识符 =====
EXPERIMENT_ID="H"  # 您可以根据需要修改这个标识符

echo "开始执行实验批次: $EXPERIMENT_ID"
echo "======================="


# # ========== 第一次训练 (loss_type=kld, k_fusion_type=add) ==========
echo "训练 1/5: add"
python -m src.model.dynamic_k.cluster_dist_transformer_dynamic_k \
  ...
  --k_fusion_type add \
  --training_mode all \
  --loss_type kld > dynamic_logs_H/add.log 2>&1


# # ========== 第二次训练 (loss_type=kld, k_fusion_type=concat) ==========
echo "训练 2/5: concat"
python -m src.model.dynamic_k.cluster_dist_transformer_dynamic_k \
  --train_queries_path input/Training_data/gist1M_learn/leafsize20K/dynamicK/train_queries.npz \
  --centroids_path input/Training_data/gist1M_learn/leafsize20K/centroids.npy \
  ...
  --training_mode all \
  --loss_type kld > dynamic_logs_H/concat.log 2>&1

# # ========== 第三次训练 (loss_type=kld, k_fusion_type=cross_attention) ==========
...
python -m src.model.dynamic_k.cluster_dist_transformer_dynamic_k \
  ...
  --training_mode all \
  --loss_type kld > dynamic_logs_H/cross_attention.log 2>&1

# # ========== 第四次训练 (loss_type=kld, k_fusion_type=gated) ==========
echo "训练 4/5: gated"
python -m src.model.dynamic_k.cluster_dist_transformer_dynamic_k \
  --train_queries_path input/Training_data/gist1M_learn/leafsize20K/dynamicK/train_queries.npz \
  --centroids_path input/Training_data/gist1M_learn/leafsize20K/centroids.npy \
  ...
  --k_fusion_type gated \
  --training_mode all \
  --loss_type kld > dynamic_logs_H/gated.log 2>&1

# # ========== 第五次训练 (loss_type=kld, k_fusion_type=adaptive) ==========
...
python -m src.model.dynamic_k.cluster_dist_transformer_dynamic_k \
  ...
  --training_mode all \
  --loss_type kld > dynamic_logs_H/adaptive.log 2>&1
